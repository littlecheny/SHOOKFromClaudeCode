"""
OKX Bitcoin 合约价格获取工具封装

通过 OKX V5 API 获取 BTC 永续合约价格和历史数据
"""
import logging
...
class GetOKXFuturesPriceTool(BaseTool):
    """获取 OKX Bitcoin 合约价格工具"""
    
    API_BASE = "https://www.okx.com"
    
    @property
    def name(self) -> str:
        return "get_okx_futures_price"
    
    @property
    def description(self) -> str:
        return "获取 OKX Bitcoin 永续合约价格和历史数据"
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "inst_id": {
                    "type": "string",
                    "description": "合约交易对，例如 BTC-USDT-SWAP",
    ...
    def execute(
        ...
        """获取 OKX Bitcoin 合约价格数据"""
        _ensure_deps()
        
        logger.info(f"获取 OKX 合约价格，inst_id={inst_id}, bar={bar}, limit={limit}")
        ...
        try:
            ...
            logger.info(f"成功获取合约价格: ${latest['close']:.2f}")
            
            return ToolResult(success=True, data=result)
        
        except Exception as e:
            logger.error(f"获取合约价格失败: {e}", exc_info=True)
            return ToolResult(success=False, error=str(e), data={'inst_id': inst_id})


class GenerateOKXFuturesDashboardTool(BaseTool):
    """生成 OKX Bitcoin 合约技术分析看板工具"""
    
    API_BASE = "https://www.okx.com"
    
    @property
    def name(self) -> str:
        return "generate_okx_futures_dashboard"
    
    @property
    def description(self) -> str:
        return "生成 OKX Bitcoin 合约技术分析看板"
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "output_path": {
                    "type": "string",
                    "default": "reports/okx_futures_dashboard.html"
                },
                "bar": {
                    "type": "string",
                    "default": "1H"
    ...
    def execute(
        self,
        inst_id: str = "BTC-USDT-SWAP",
        output_path: str = "reports/okx_futures_dashboard.html",
        bar: str = "1H",
        limit: int = 168
    ) -> ToolResult:
        """生成合约技术分析看板"""
        ...
        logger.info(f"生成 OKX 合约看板: {inst_id}")
        ...
        try:
            ...
            fig.update_layout(
                title=f'{inst_id} 合约技术分析看板<br><sub>OKX | {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</sub>',
                height=1200,
            ...
            logger.info(f"生成合约看板成功: {output_file}")
            return ToolResult(success=True, data=result)
        
        except Exception as e:
            logger.error(f"生成合约看板失败: {e}", exc_info=True)
            return ToolResult(success=False, error=str(e), data={'inst_id': inst_id})
