...
def demo_basic_usage():
    """演示 1: 基本使用"""
    print("=" * 60)
    print("演示 1: 基本使用 - 获取当天 DXY 数据")
    print("=" * 60)
    print()
    
    registry = get_default_registry()
    tool = registry.get('get_dxy')
    ...
    if result.success:
        ...
        print(f"  涨跌: {data['change']:+.4f} ({data['change_percent']:+.2f}%)")
...
def demo_historical_data():
    ...
    if result.success:
        ...
        print(f"  总变动: {trend_change:+.4f} ({trend_percent:+.2f}%)")
...
def demo_info_query():
    """演示 3: 获取基本信息"""
    print("=" * 60)
    print("演示 3: 获取基本信息 - 查询 DXY 指数详情")
    print("=" * 60)
    print()
    
    registry = get_default_registry()
    tool = registry.get('get_dxy_info')
...
def demo_multiple_periods():
    ...
    print(f"{'周期':<10} {'涨跌':<10} {'涨跌幅':<10} {'数据点数'}")
    print("-" * 50)
    ...
    for period, label in periods:
        ...
        if result.success:
            ...
            print(f"{label:<10} {change:+7.4f}    {change_percent:+6.2f}%    {len(history)} 天")
        else:
