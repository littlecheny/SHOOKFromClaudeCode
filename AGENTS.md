...
- 界面：两个标签 `instruments` / `telemetry`（呼应菜单栏直升机图标的座舱主题，标签枚举见 `gui/src/components/TabBar.tsx` 的 `Screen` 类型）。
  - `instruments`（`components/status/StatusScreen.tsx`）：focus / todos（`FocusCard`、`TodoList`）+ 功能区（`components/workflows/WorkflowLauncher.tsx`）。功能区可点击运行 `get-news` / `predict-btc`（白名单见 `gui/electron/ipc.ts` 的 `WORKFLOW_ALLOWLIST`；`runway`/`cockpit`/`goal` 是 TUI 交互式命令，不在 GUI 暴露），每张 `WorkflowCard` 带实时日志流 + 最近运行状态，完成后发系统 Notification。
  - `telemetry`（`components/telemetry/TelemetryScreen.tsx`）：四千周人生视图（`components/status/LifeGrid.tsx` + `gui/electron/lifeWeeks.ts`，出生日期写死 `2002-06-04`；单日 commit 是心情记录，真实数据按月读 `.shook/moods/YYYY-MM.json`，没有任何 mood 月文件时才回退到 `MockCommitSource`）。`LifeGrid` 有「总览」（52 列全景）和「100 周」（分页放大）两种视图模式，点击周格下钻到 7 天心情明细，天格 hover 才显示 note。
...
- IPC 面集中注册在 `gui/electron/ipc.ts`：只读 `state:get`/`lifeweeks:get`/`workflow:list`，写 `state:setFocus`/`state:addTodo`/`state:toggleTodo`/`state:removeTodo`/`workflow:run`，推送事件 `state:changed`/`workflow:log`/`workflow:status`/`ui:navigate`。`ui:navigate` 的目标名在 `App.tsx` 里归一化：`instruments`/`status`/`workflows` → instruments 页，`telemetry`/`life` → telemetry 页（工作流完成的通知点击跳 `instruments`）。`contextIsolation: true`、`nodeIntegration: false`。
- UI 设计原则：遵循本机 `minimalist-ui` skill 协议（暖白画布、1px 边框、无重阴影/渐变、衬线大标题、8-12px 圆角）。色彩 token 定义在 `gui/src/index.css` 的 `@theme` 里；四千周热力图使用八类心情角色色带（`sponge`/`patrick`/`squid`/`plankton`/`krabs`/`gary`/`sandy`/`puff` 的 `-1..-4`），只做 spot accent，不大面积铺色。改视觉风格前先看这份 token 定义，不要引入新的配色系统。
...
| `.shook/moods/YYYY-MM.json` | 四千周心情 commit 月文件；格式见 `docs/examples/shook/moods.example.json` |
